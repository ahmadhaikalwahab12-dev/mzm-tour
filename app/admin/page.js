...
const emptyJadwal = { nama_paket:"", jenis:"Umroh", tanggal_berangkat:"", tanggal_pulang:"", maskapai:"", hotel:"", kuota:"", terisi:"0", status:"Aktif" };
...
export default function AdminPage() {
  ...
  const handleSave = async () => {
    ...
    showToast(modal.mode === "add" ? "Berhasil ditambahkan!" : "Berhasil diperbarui!");
    closeModal(); fetchAll();
  };
  ...
  const toggleTampil = async (item) => {
    await supabase.from("testimoni").update({ tampil: !item.tampil }).eq("id", item.id);
    fetchAll();
  };
  ...
  // LOGIN PAGE 
  if (!loggedIn) return (
    <div className="min-h-screen bg-gradient-to-br from-[#042f2f] to-[#008080] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8">
        <div className="text-center mb-8">
          <Image src="/icon/logo.png" width={80} height={55} alt="logo" className="mx-auto mb-4" />
          <h1 className="text-xl font-extrabold text-gray-900">Admin Panel</h1>
          <p className="text-gray-400 text-sm mt-1">MZM Tour & Travel</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input value={loginForm.user} onChange={e=>setLoginForm({...loginForm,user:e.target.value})}
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#008080] transition-colors"
              placeholder="admin" />
          </div>
          <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              <h2 className="font-extrabold text-gray-800 flex items-center gap-2">
                <Image src="/icon/pembimbing.png" alt="" width={16} height={16} className="object-contain opacity-60" />
                Daftar Paket
                ...
                <div key={p.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                  <div className="w-12 h-12 bg-gray-100 rounded-xl overflow-hidden shrink-0 flex items-center justify-center">
                    {p.gambar
                      ...
                      : <Image src="/icon/pembimbing.png" alt="" width={24} height={24} className="object-contain opacity-30" />
                    }
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <select value={p.status} onChange={async e => { await supabase.from("paket").update({status:e.target.value}).eq("id",p.id); fetchAll(); }}
            ...
            <div className="mt-6 bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              <p className="text-xs font-semibold text-gray-500 mb-3 flex items-center gap-1.5">
                <image src= "/icon/fasilitas.png" alt="" width={14} height={14} className="object-contain brightness-0 invert" ></image>
                Preview
              </p>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-extrabold text-gray-800 flex items-center gap-2">
                ...
                <span className="bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full">{jadwalList.length}</span>
              </h2>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-50 shadow-sm">
              {jadwalList.length === 0 ? (
                <div className="py-16 text-center text-gray-400 text-sm">Belum ada jadwal</div>
              ) : jadwalList.map(j => (
                <div key={j.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                  <div className="w-12 h-12 bg-teal-50 rounded-xl flex items-center justify-center shrink-0">
                    <Image src="/icon/calender1.png" alt="" width={24} height={24} className="object-contain opacity-70" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${j.status==="Aktif"?"bg-green-100 text-green-700":"bg-gray-100 text-gray-500"}`}>
                        {j.status}
                      </span>
                    </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              {modal.type === "paket" && <>
                ...
                <Field label="URL Gambar (opsional)" value={form.gambar} onChange={v=>setForm({...form,gambar:v})} placeholder="https://..." />
