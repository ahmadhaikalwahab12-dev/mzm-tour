...
export default function CekJadwal() {
  ...
  const filtered = jadwalData.filter((j) => {
    const matchBulan   = bulan   === "Semua Bulan"   || j.bulan  === bulan;
    ...
    <div className="min-h-screen pt-24 sm:pt-28 md:pt-32 px-4 sm:px-8 md:px-10 pb-20" style={{ background: "#edf7f5" }}>
      <div className="max-w-screen-lg mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 items-end">
            ...
            <div>
              ...
              <select value={layanan} onChange={(e) => setLayanan(e.target.value)}
                className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-[#008080]">
                {["Semua Layanan","Umroh","Haji","Wisata"].map((l) => <option key={l}>{l}</option>)}
              </select>
            </div>
            <div>
            ...
            <button className="w-full flex items-center justify-center bg-[#008080] hover:bg-[#006666] text-white font-semibold text-sm px-6 py-2.5 rounded-lg transition-colors duration-200">
              Cari Jadwal
            </button>
          </div>
        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                  ...
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="text-center shrink-0 w-12 sm:w-14">
                      <p className="text-[#008080] text-[10px] font-bold uppercase">{j.bulan}</p>
                      <p className="text-[#008080] text-3xl sm:text-4xl font-extrabold leading-none">{j.tanggal}</p>
                      <p className="text-gray-400 text-[10px]">{j.tahun}</p>
                    </div>
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="font-bold text-gray-900 text-sm sm:text-base mb-1 leading-snug">{j.nama}</h3>
                      <div className="flex items-center gap-3 text-gray-400 text-[10px] sm:text-xs flex-wrap">
                        <span>▷ {j.maskapai}</span>
                        <span>□ {j.hotel}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Tombol WA  */}
                  <Link
                    href={`https://wa.me/6282311000853?text=${encodeURIComponent(`Halo, saya ingin tanya tentang jadwal ${j.nama}`)}`}
                    target="_blank"
        ...
        <button
          ...
          className="flex items-center gap-2 text-gray-600 text-sm font-semibold hover:text-[#008080] transition-colors duration-200 mb-12"
        >
          <span className="text-base">{showRiwayat ? "∨" : "›"}</span>
          Riwayat Keberangkatan ({jadwalData.length} jadwal)
        </button>
